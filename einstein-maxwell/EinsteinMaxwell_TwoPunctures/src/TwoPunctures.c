                       ...
                       derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = EinsteinMaxwell_Index(ivar,i,j,k,1,n1,n2,n3);
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          indx = EinsteinMaxwell_Index(ivar,i,j,k,1,n1,n2,n3);
  ...
  if (do_initial_debug_output && CCTK_MyProc(cctkGH) == 0)
  {
    ...
    for (ivar = 0; ivar < nvar; ivar++)
      for (i = 0; i < n1; i++)
        for (j = 0; j < n2; j++)
        {
          ...
          B = -cos (be);
          phi = 0.0;
          indx = EinsteinMaxwell_Index(ivar,i,j,0,1,n1,n2,n3);
        ...
        EinsteinMaxwell_AB_To_XR (nvar, A, B, &X, &R, U);
        /* Calculation of (x,r)*/
        EinsteinMaxwell_C_To_c (nvar, X, R, &(s_x[indx]), &r, U);
        /* Calculation of (y,z)*/
        EinsteinMaxwell_rx3_To_xyz (nvar, s_x[i3D], r, phi, &(s_y[indx]), &(s_z[indx]), U);
        fprintf(debug_file,
    ...
    {
      CCTK_REAL d;
      indx = EinsteinMaxwell_Index(0,0,i,0,1,n1,n2,n3);
      d = EinsteinMaxwell_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,
              s_x[indx], 0.0, 0.0);
    ...
    {
      CCTK_REAL d;
      int ip= EinsteinMaxwell_Index(0,0,i+1,0,1,n1,n2,n3);
      indx = EinsteinMaxwell_Index(0,0,i,0,1,n1,n2,n3);
      for (j=-10; j<10; j++)
      {
        d = EinsteinMaxwell_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,
...
EinsteinMaxwell_TwoPunctures (CCTK_ARGUMENTS)
{
  ...
  * mm = par_m_minus;
  ...
  if (! F) {
    ...
    EinsteinMaxwell_allocate_derivs (&u, ntotal);
    EinsteinMaxwell_allocate_derivs (&v, ntotal);
    EinsteinMaxwell_allocate_derivs (&cf_v, ntotal);
    ...
    if(!(give_bare_mass)) {
      ...
      do {
        CCTK_VInfo (CCTK_THORNSTRING, "Bare masses: mp=%.15g, mm=%.15g",
                    (double)*mp, (double)*mm);
        EinsteinMaxwell_Newton (cctkGH, nvar, n1, n2, n3, v, Newton_tol, 1);

        EinsteinMaxwell_F_of_v (cctkGH, nvar, n1, n2, n3, v, F, u);

        up = EinsteinMaxwell_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v, par_b, 0., 0.);
        um = EinsteinMaxwell_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,-par_b, 0., 0.);
    ...
    EinsteinMaxwell_Newton (cctkGH, nvar, n1, n2, n3, v, Newton_tol, Newton_maxit);
 
    EinsteinMaxwell_F_of_v (cctkGH, nvar, n1, n2, n3, v, F, u);

    EinsteinMaxwell_SpecCoef(n1, n2, n3, nvar, v.d0, cf_v.d0);
    ...
    *mm_adm = (1 + um) * *mm + *mp * *mm / (4. * par_b);
    ...
    admMass = (*mp + *mm
               - 4*par_b*EinsteinMaxwell_PunctEvalAtArbitPosition(v.d0, 0, 1, 0, 0, nvar, n1, n2, n3));
  ...
  for (int k = imin[2]; k < imax[2]; ++k)
  {
    for (int j = imin[1]; j < imax[1]; ++j)
    {
      for (int i = imin[0]; i < imax[0]; ++i)
      {
        ...
        EinsteinMaxwell_Calc_EM_Fields(xx, yy, zz, Ei, Bi, &F2);

        CCTK_REAL U;
        CCTK_REAL charge_potential_sq;
        switch (gsm)
        {
        case GSM_Taylor_expansion:
          U = EinsteinMaxwell_PunctTaylorExpandAtArbitPosition
            (0, nvar, n1, n2, n3, v, xx, yy, zz);
          break;
        case GSM_evaluation:
          U = EinsteinMaxwell_PunctIntPolAtArbitPositionFast(0, nvar, n1, n2, n3, cf_v, xx, yy, zz);
          break;
        default:
          assert (0);
        }
...
#define EXTEND(M,r) \
        ...
        CCTK_REAL Aij[3][3];
        EinsteinMaxwell_BY_Aijofxyz (xx, yy, zz, Aij);
        ...
        /* Einstein-Maxwell variables.
           ...
           scalars are initially zero. */

        Ex[ind]    = Ei[0] / pow(psi1, 6);
        Ey[ind]    = Ei[1] / pow(psi1, 6);
        Ez[ind]    = Ei[2] / pow(psi1, 6);

        Bx[ind]    = Bi[0] / pow(psi1, 6);
  ...
  if (0) {
    ...
    EinsteinMaxwell_free_derivs (&u, ntotal);
    EinsteinMaxwell_free_derivs (&v, ntotal);
    EinsteinMaxwell_free_derivs (&cf_v, ntotal);
  }
}
