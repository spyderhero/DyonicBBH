...
subroutine qlm_em_killing_normalise (CCTK_ARGUMENTS, hn)
  use cctk
  use qlm_em_killing_normalisation
  implicit none
  ...
  do ii=1,ngeodesics
     i = 1 + qlm_em_nghoststheta(hn) &
          + ii * (qlm_em_ntheta(hn) - 2*qlm_em_nghoststheta(hn) - 1) / (ngeodesics+1)
     if (qlm_em_xi_p(i,j,hn) < 0) then
        qlm_em_xi_t(:,:,hn) = -qlm_em_xi_t(:,:,hn)
        qlm_em_xi_p(:,:,hn) = -qlm_em_xi_p(:,:,hn)
        qlm_em_chi (:,:,hn) = -qlm_em_chi (:,:,hn)
     end if
  ...
  qlm_em_xi_t(:,:,hn) = qlm_em_xi_t(:,:,hn) * factor
  qlm_em_xi_p(:,:,hn) = qlm_em_xi_p(:,:,hn) * factor
  qlm_em_chi (:,:,hn) = qlm_em_chi (:,:,hn) * factor
...
end subroutine qlm_em_killing_normalise
