...
subroutine qlm_em_killing_gradient (CCTK_ARGUMENTS, hn)
  use cctk
  use constants
  use qlm_em_boundary
  use qlm_em_derivs
  ...
  do j = 1+qlm_em_nghostsphi(hn), qlm_em_nphi(hn)-qlm_em_nghostsphi(hn)
     do i = 1+qlm_em_nghoststheta(hn), qlm_em_ntheta(hn)-qlm_em_nghoststheta(hn)
        ...
        dpsi2(1) = (abs2(qlm_em_psi2(i+1,j,hn)) - abs2(qlm_em_psi2(i-1,j,hn))) / (2*qlm_em_delta_theta(hn))
        dpsi2(2) = (abs2(qlm_em_psi2(i,j+1,hn)) - abs2(qlm_em_psi2(i,j-1,hn))) / (2*qlm_em_delta_phi(hn))
        
#if 0
        ddpsi2(1,1) = (abs2(qlm_em_psi2(i+1,j,hn)) - 2*abs2(qlm_em_psi2(i,j,hn)) + abs2(qlm_em_psi2(i-1,j,hn))) / qlm_em_delta_theta(hn)**2
        ddpsi2(2,2) = (abs2(qlm_em_psi2(i,j+1,hn)) - 2*abs2(qlm_em_psi2(i,j,hn)) + abs2(qlm_em_psi2(i,j-1,hn))) / qlm_em_delta_phi(hn)**2
        ddpsi2(1,1) = (abs2(qlm_em_psi2(i-1,j-1,hn)) - abs2(qlm_em_psi2(i+1,j-1,hn)) - abs2(qlm_em_psi2(i-1,j+1,hn)) + abs2(qlm_em_psi2(i+1,j+1,hn))) / (4*qlm_em_delta_theta(hn)*qlm_em_delta_phi(hn))
  ...
  call em_set_boundary (CCTK_PASS_FTOF, hn, qlm_em_xi_t(:,:,hn), -1)
  call em_set_boundary (CCTK_PASS_FTOF, hn, qlm_em_xi_p(:,:,hn), -1)
  ...
  do j = 1+qlm_em_nghostsphi(hn), qlm_em_nphi(hn)-qlm_em_nghostsphi(hn)
     do i = 1+qlm_em_nghoststheta(hn), qlm_em_ntheta(hn)-qlm_em_nghoststheta(hn)
        ...
        if (sum(xi**2) < 1.0d-4**2) then
           ...
           qlm_em_xi_t(i,j,hn) = sum(qlm_em_xi_t(i:i+1,j:j+1,hn)) / 4
           qlm_em_xi_p(i,j,hn) = sum(qlm_em_xi_p(i:i+1,j:j+1,hn)) / 4
  ...
  do j = 1+qlm_em_nghostsphi(hn), qlm_em_nphi(hn)-qlm_em_nghostsphi(hn)
     do i = 1+qlm_em_nghoststheta(hn), qlm_em_ntheta(hn)-qlm_em_nghoststheta(hn)
        ...
        qlm_em_chi(i,j,hn) = chi1
        
     end do
  end do
  
  call em_set_boundary (CCTK_PASS_FTOF, hn, qlm_em_chi (:,:,hn), +1)
  
end subroutine qlm_em_killing_gradient
