...
module qlm_em_killing_normalisation
  use cctk
  use constants
  implicit none
...
contains
  ...
  subroutine killing_geodesic &
    ...
    org_theta = qlm_em_origin_theta(hn)
    org_phi   = qlm_em_origin_phi(hn)
    del_theta = qlm_em_delta_theta(hn)
    del_phi   = qlm_em_delta_phi(hn)
    ...
    dtheta = killing_interp (qlm_em_xi_t(:,:,hn), &
         org_theta, org_phi, del_theta, del_phi, theta, phi, ierr1)
    dphi   = killing_interp (qlm_em_xi_p(:,:,hn), &
    ...
    dlambda = (qlm_em_delta_phi(hn) / dphi) / 2
...
end module qlm_em_killing_normalisation
