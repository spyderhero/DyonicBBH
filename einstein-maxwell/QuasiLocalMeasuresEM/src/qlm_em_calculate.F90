...
subroutine qlm_em_calculate (CCTK_ARGUMENTS)
  use cctk
  ...
  do pass = 1, (num_surfaces + num_procs - 1) / num_procs
     ...
     if (hn > 0) then
        if (cctk_time < begin_qlm_em_calculations_after(hn)) hn = 0
     end if
     ...
     if (hn > 0) then
        call qlm_em_import_surface (CCTK_PASS_FTOF, hn)
        if (qlm_em_calc_error(hn) /= 0) hn = 0
     endif
     
     if (hn > 0) then
        call qlm_em_set_coordinates (CCTK_PASS_FTOF, hn)
     end if
     ...
     call qlm_em_interpolate (CCTK_PASS_FTOF, hn)
     
     if (hn > 0) then
        if (qlm_em_calc_error(hn) /= 0) goto 9999
        
        call qlm_em_calc_tetrad (CCTK_PASS_FTOF, hn)
        call qlm_em_calc_newman_penrose (CCTK_PASS_FTOF, hn)
        call qlm_em_calc_weyl_scalars (CCTK_PASS_FTOF, hn)
        call qlm_em_calc_twometric (CCTK_PASS_FTOF, hn)
        if (CCTK_EQUALS(killing_vector_method, "axial")) then
           call qlm_em_killing_axial (CCTK_PASS_FTOF, hn)
        else if (CCTK_EQUALS(killing_vector_method, "eigenvector")) then
           call qlm_em_killing_transport (CCTK_PASS_FTOF, hn)
           if (qlm_em_calc_error(hn) /= 0) goto 9999
           call qlm_em_killing_normalise (CCTK_PASS_FTOF, hn)
        else if (CCTK_EQUALS(killing_vector_method, "gradient")) then
           call qlm_em_killing_gradient (CCTK_PASS_FTOF, hn)
           call qlm_em_killing_normalise (CCTK_PASS_FTOF, hn)
        else
        ...
        if (qlm_em_have_killing_vector(hn) /= 0) then
           call qlm_em_multipoles (CCTK_PASS_FTOF, hn)
           call qlm_em_multipoles_normalise (CCTK_PASS_FTOF, hn)
        end if
        ...
        if (output_vtk_every /= 0) then
           if (mod (cctk_iteration, output_vtk_every) == 0) then
              ...
              call qlm_em_output_vtk (CCTK_PASS_FTOF, hn, &
...
9999    continue
        ...
        if (qlm_em_timederiv_order(hn) < 2) then
...
end subroutine qlm_em_calculate
