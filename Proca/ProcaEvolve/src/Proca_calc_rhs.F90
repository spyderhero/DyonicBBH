...
subroutine Proca_calc_rhs( CCTK_ARGUMENTS )
  ...
  do i = 1+cctk_nghostzones(1), cctk_lsh(1)-cctk_nghostzones(1)
    ...
    if (derivs_order == 4) then
      ...
      call CCTK_WARN(0, "derivs_order not implemented in Proca_calc_rhs_EM_direct_corrected.F90")
    end if

    if (use_jacobian) then
      call Proca_d1_Scalar_apply_jacobian(d1_ch, jac)
      call Proca_d1_Scalar_apply_jacobian(d1_alph, jac)
      call Proca_d1_Scalar_apply_jacobian(d1_lPsi, jac)
      call Proca_d1_Scalar_apply_jacobian(d1_lPhi, jac)
      call Proca_d1_Vector_apply_jacobian(d1_beta, jac)
      call Proca_d1_Vector_apply_jacobian(d1_lE, jac)
      call Proca_d1_Vector_apply_jacobian(d1_lB, jac)
      call Proca_d1_2nd_Tensor_apply_jacobian(d1_hh, jac, 1)
    end if
...
subroutine Proca_calc_rhs_bdry( CCTK_ARGUMENTS )
  ...
  ierr = NewRad_Apply(cctkGH, Psi, rhs_Psi, Psi0, one, n_Psi)
  ierr = NewRad_Apply(cctkGH, Phi, rhs_Phi, Phi0, one, n_Phi)
