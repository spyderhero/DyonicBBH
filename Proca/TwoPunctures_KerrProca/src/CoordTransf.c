/* TwoPunctures_KerrEinsteinMaxwell:  File  "CoordTransf.c"*/
...
/*-----------------------------------------------------------*/
void
TPKP_AB_To_XR (int nvar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL *X, CCTK_REAL *R,
	  derivs U)
// Calculates the coordinates (X,R) from the spectral coordinates (A,B):
//       X = 2*artanh[(A+1)/2],  R = Pi/2 + 2*arctan(B),  A,B \in [-1,1]
// (the coordinate A of 'gr-qc/0404056' runs in [0,1] and is 'At' here; xi=X, eta=R)
//
// MZ: in this version we're changing the 'X' coordinate
// transformation to 
//           X = 2 arctanh{ sinh( lambda At ) / sinh(lambda) }
...
/*                          U.d22[]=U[]_RR; U.d23[]=U[]_R3; U.d33[]=U[]_33; */
{
  ...
  CCTK_REAL 
    At=0.5*(A+1.), // the coordinate 'A' of 'gr-qc/0404056'
    A_X, A_XX, B_R, B_RR;
  CCTK_INT ivar ;
  
  // MZ: changed 
  if( lambda > 1.0e-4 ) {
    At = sinh(lambda * At) / sinh(lambda) ;
  } else {
    At = At - lambda*lambda/6. * At * (1. - At*At) ;
  }
  ...
  *R = Pih+2.*atan(B);
  ...
  // MZ: changed
  A_X  = 2. / ( (1.+cosh(*X)) * sqrt(1.+sinh(lambda)*sinh(lambda) 
                                     * tanh(0.5*(*X))*tanh(0.5*(*X)) ) );
  A_XX = - (1.+cosh(*X)*cosh(lambda)*cosh(lambda)) 
    * pow(sech(0.5*(*X)),4) * tanh(0.5*(*X)) / 
    (2.*pow(1.+sinh(lambda)*sinh(lambda)*tanh(0.5*(*X))*tanh(0.5*(*X)), 3./2.)) ;
  if( lambda > 1.0e-4 ) {
    A_X  = A_X * sinh(lambda)/lambda ;
    A_XX = A_XX * sinh(lambda)/lambda ;
  } else {
    A_X   = A_X * ( 1. + 1./6.*lambda*lambda 
                    + 1./120. * lambda*lambda*lambda*lambda  ) ;
    A_XX  = A_XX * ( 1. + 1./6.*lambda*lambda 
                     + 1./120. * lambda*lambda*lambda*lambda  ) ;
  }
...
/*-----------------------------------------------------------*/
void
TPKP_C_To_c (int nvar, CCTK_REAL X, CCTK_REAL R, CCTK_REAL *x, CCTK_REAL *r,
	derivs U)
...
/*-----------------------------------------------------------*/
void
TPKP_rx3_To_xyz (int nvar, CCTK_REAL x, CCTK_REAL r, CCTK_REAL phi,
