/* TwoPunctures_KerrEinsteinMaxwell:  File  "Newton.c"*/
              ...
              CCTK_REAL const * restrict const * restrict const JFD)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    for (i = 0; i < n1; i++)
    {
      Ip = TPKP_Index (ivar, i + 1, j, k, nvar, n1, n2, n3);
      Ic = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
      Im = TPKP_Index (ivar, i - 1, j, k, nvar, n1, n2, n3);
              ...
              CCTK_REAL const * restrict const * restrict const JFD)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    for (j = 0; j < n2; j++)
    {
      Ip = TPKP_Index (ivar, i, j + 1, k, nvar, n1, n2, n3);
      Ic = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
      Im = TPKP_Index (ivar, i, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
TPKP_TestRelax (CCTK_POINTER_TO_CONST cctkGH,
	   ...
	   CCTK_REAL *dv)
{
  ...
  TPKP_SetMatrix_JFD (nvar, n1, n2, n3, u, ncols, cols, JFD);
...
/*      sh  = dvector(0, ntotal-1);*/
  ...
  TPKP_J_times_dv (nvar, n1, n2, n3, dv, r, u);
#pragma omp parallel for
  for (int j = 0; j < ntotal; j++)
    rt[j] = r[j] = F[j] - r[j];
...
#pragma omp parallel for
    ...
    for (int j = 0; j < NRELAX; j++)	/* solves JFD*ph = p by relaxation*/
      relax (ph.d0, nvar, n1, n2, n3, p, ncols,
                 (const int * const restrict* const restrict)cols,
                 (const CCTK_REAL * const restrict* const restrict)JFD);
    TPKP_J_times_dv (nvar, n1, n2, n3, ph, vv, u);	/* vv=J*ph*/
...
#pragma omp parallel for
    ...
    TPKP_J_times_dv (nvar, n1, n2, n3, sh, t, u);	/* t=J*sh*/
...
/* -------------------------------------------------------------------*/
void
TPKP_Newton (CCTK_POINTER_TO_CONST const cctkGH,
        int const nvar, int const n1, int const n2, int const n3,
	derivs v,
