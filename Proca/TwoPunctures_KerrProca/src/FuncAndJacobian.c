/* TwoPunctures_KerrEinsteinMaxwell:  File  "FuncAndJacobian.c"*/
...
/* --------------------------------------------------------------------------*/
int
TPKP_Index (int ivar, int i, int j, int k, int nvar, int n1, int n2, int n3)
{
  int i1 = i, j1 = j, k1 = k;
...
/* --------------------------------------------------------------------------*/
void
TPKP_allocate_derivs (derivs * v, int n)
{
  int m = n - 1;
  (*v).d0 = dvector (0, m);
  (*v).d1 = dvector (0, m);
...
/* --------------------------------------------------------------------------*/
void
TPKP_free_derivs (derivs * v, int n)
{
  int m = n - 1;
...
/* --------------------------------------------------------------------------*/
void
TPKP_Derivatives_AB3 (int nvar, int n1, int n2, int n3, derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
	...
	for (i = 0; i < n1; i++)
	{
	  indx[i] = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[i] = v.d0[indx[i]];
	}
	chebft_Zeros (p, n1, 0);
	chder (p, dp, n1);
	...
	for (j = 0; j < n2; j++)
	{
	  indx[j] = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[j] = v.d0[indx[j]];
	  q[j] = v.d1[indx[j]];
	}
	chebft_Zeros (p, n2, 0);
	...
	for (k = 0; k < n3; k++)
	{
	  indx[k] = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[k] = v.d0[indx[k]];
	  q[k] = v.d1[indx[k]];
	  r[k] = v.d2[indx[k]];
	}
...
/* --------------------------------------------------------------------------*/
void
TPKP_F_of_v (CCTK_POINTER_TO_CONST cctkGH,
  ...
  if (use_sources)
  {
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = TPKP_Index(0,i,j,k,1,n1,n2,n3);
          ...
          for (ivar = 0; ivar < nvar; ivar++)
          {
            indx = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
          ...
          TPKP_AB_To_XR (nvar, A, B, &X, &R, U);
          /* Calculation of (x,r) and*/
          /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
          TPKP_C_To_c (nvar, X, R, &(s_x[i3D]), &r, U);
          /* Calculation of (y,z) and*/
          /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
          TPKP_rx3_To_xyz (nvar, s_x[i3D], r, phi, &(s_y[i3D]), &(s_z[i3D]), U);
        }
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
          sources[TPKP_Index(0,i,j,k,1,n1,n2,n3)]=0.0;

  TPKP_Derivatives_AB3 (nvar, n1, n2, n3, v);
  ...
  for (i = 0; i < n1; i++)
  {
    for (j = 0; j < n2; j++)
    {
      for (k = 0; k < n3; k++)
      {
        ...
        /* Calculation of (x,r) and*/
        /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
        TPKP_C_To_c (nvar, X, R, &x, &r, U);
        /* Calculation of (y,z) and*/
        /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
        TPKP_rx3_To_xyz (nvar, x, r, phi, &y, &z, U);
        TPKP_NonTPKP_LinEquations (sources[TPKP_Index(0,i,j,k,1,n1,n2,n3)],
...
/*                      0.125 * BY_KKofxyz (x, y, z) / psi7 +*/
  ...
  TPKP_free_derivs (&U, nvar);
}

/* --------------------------------------------------------------------------*/
void
TPKP_J_times_dv (int nvar, int n1, int n2, int n3, derivs dv,
...
#pragma omp parallel for private (values,dU,U,i,j,k,al,A,be,B,phi,X,R,x,r,y,z,Am1,ivar,indx) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	TPKP_AB_To_XR (nvar, A, B, &X, &R, dU);
	/* Calculation of (x,r) and*/
	/* (dU, dU_x, dU_r, dU_3, dU_xx, dU_xr, dU_x3, dU_rr, dU_r3, dU_33)*/
	TPKP_C_To_c (nvar, X, R, &x, &r, dU);
	/* Calculation of (y,z) and*/
	/* (dU, dU_x, dU_y, dU_z, dU_xx, dU_xy, dU_xz, dU_yy, dU_yz, dU_zz)*/
	TPKP_rx3_To_xyz (nvar, x, r, phi, &y, &z, dU);
	TPKP_LinEquations (A, B, X, R, x, r, phi, y, z, dU, U, values);
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  indx = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
TPKP_JFD_times_dv (int i, int j, int k, int nvar, int n1, int n2,
	      int n3, derivs dv, derivs u, CCTK_REAL *values)
{				/* Calculates rows of the vector 'J(FD)*dv'.*/
  /* First row to be calculated: row = TPKP_Index(0,      i, j, k; nvar, n1, n2, n3)*/
  /* Last  row to be calculated: row = TPKP_Index(nvar-1, i, j, k; nvar, n1, n2, n3)*/
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    int iccc = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3),
      ipcc = TPKP_Index (ivar, i + 1, j, k, nvar, n1, n2, n3),
      imcc = TPKP_Index (ivar, i - 1, j, k, nvar, n1, n2, n3),
      icpc = TPKP_Index (ivar, i, j + 1, k, nvar, n1, n2, n3),
      icmc = TPKP_Index (ivar, i, j - 1, k, nvar, n1, n2, n3),
      iccp = TPKP_Index (ivar, i, j, k + 1, nvar, n1, n2, n3),
      iccm = TPKP_Index (ivar, i, j, k - 1, nvar, n1, n2, n3),
      icpp = TPKP_Index (ivar, i, j + 1, k + 1, nvar, n1, n2, n3),
      icmp = TPKP_Index (ivar, i, j - 1, k + 1, nvar, n1, n2, n3),
      icpm = TPKP_Index (ivar, i, j + 1, k - 1, nvar, n1, n2, n3),
      icmm = TPKP_Index (ivar, i, j - 1, k - 1, nvar, n1, n2, n3),
      ipcp = TPKP_Index (ivar, i + 1, j, k + 1, nvar, n1, n2, n3),
      imcp = TPKP_Index (ivar, i - 1, j, k + 1, nvar, n1, n2, n3),
      ipcm = TPKP_Index (ivar, i + 1, j, k - 1, nvar, n1, n2, n3),
      imcm = TPKP_Index (ivar, i - 1, j, k - 1, nvar, n1, n2, n3),
      ippc = TPKP_Index (ivar, i + 1, j + 1, k, nvar, n1, n2, n3),
      impc = TPKP_Index (ivar, i - 1, j + 1, k, nvar, n1, n2, n3),
      ipmc = TPKP_Index (ivar, i + 1, j - 1, k, nvar, n1, n2, n3),
      immc = TPKP_Index (ivar, i - 1, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
TPKP_SetMatrix_JFD (int nvar, int n1, int n2, int n3, derivs u,
	       int *ncols, int **cols, CCTK_REAL **Matrix)
{
  ...
  values = dvector (0, nvar - 1);
  TPKP_allocate_derivs (&dv, ntotal);
...
#pragma omp parallel for private (i,j,k,ivar,row) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  row = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  ncols[row] = 0;
	  dv.d0[row] = 0;
	}
      }
  ...
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  column = TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  dv.d0[column] = 1;
...
/*					i_0 = 0;
	  ...
	  for (i1 = i_0; i1 <= i_1; i1++)
	  {
	    for (j1 = j_0; j1 <= j_1; j1++)
	    {
	      ...
	      {
		TPKP_JFD_times_dv (i1, j1, k1, nvar, n1, n2, n3,
			      dv, u, values);
		for (ivar1 = 0; ivar1 < nvar; ivar1++)
		{
		  if (values[ivar1] != 0)
		  {
		    row = TPKP_Index (ivar1, i1, j1, k1, nvar, n1, n2, n3);
		    mcol = ncols[row];
...
CCTK_REAL
TPKP_PunctEvalAtArbitPosition (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi,
...
/* --------------------------------------------------------------------------*/
void
TPKP_calculate_derivs (int i, int j, int k, int ivar, int nvar, int n1, int n2,
		  int n3, derivs v, derivs vv)
{
  ...
  vv.d0[0] = v.d0[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d1[0] = v.d1[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d2[0] = v.d2[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d3[0] = v.d3[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d11[0] = v.d11[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_al
    + v.d1[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * cos_al;
  vv.d12[0] =
    v.d12[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al * sin_be;
  vv.d13[0] = v.d13[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d22[0] = v.d22[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_be
    + v.d2[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * cos_be;
  vv.d23[0] = v.d23[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d33[0] = v.d33[TPKP_Index (ivar, i, j, k, nvar, n1, n2, n3)];
}

/* --------------------------------------------------------------------------*/
CCTK_REAL
TPKP_interpol (CCTK_REAL a, CCTK_REAL b, CCTK_REAL c, derivs v)
{
  return v.d0[0]
...
CCTK_REAL
TPKP_PunctTaylorExpandAtArbitPosition (int ivar, int nvar, int n1,
                                  ...
                                  CCTK_REAL z)
{
  ...
  //  A = 2 * tanh (0.5 * X) - 1;
  A = tanh(0.5*X); // At
  // MZ: modified to accommodate higher resolution at punctures
  if( lambda > 1.0e-4 ) {
    A = asinh(sinh(lambda) * A) / lambda ;
  } else {
    A = A + lambda*lambda/6. * A * (1. - A*A) ;
  }
  A = 2*A - 1. ;    // A
  ...
  TPKP_calculate_derivs (i, j, k, ivar, nvar, n1, n2, n3, v, vv);
  result = TPKP_interpol (a, b, c, vv);
  TPKP_free_derivs (&vv, 1);
...
/* Calculates the value of v at an arbitrary position (x,y,z)*/
CCTK_REAL
TPKP_PunctIntPolAtArbitPosition (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  ...
  result = TPKP_PunctEvalAtArbitPosition (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
CCTK_REAL 
TPKP_TPKP_PunctEvalAtArbitPositionFast (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi, int nvar, int n1, int n2, int n3)
{
  int i, j, k, N;
  CCTK_REAL *p, *values1, **values2, result;
  // VASILIS: Nothing should be changed in this routine. This is used by TPKP_TPKP_PunctIntPolAtArbitPositionFast
...
/* --------------------------------------------------------------------------*/
CCTK_REAL
TPKP_TPKP_PunctIntPolAtArbitPositionFast (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  ...
  result = TPKP_TPKP_PunctEvalAtArbitPositionFast (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
void TPKP_SpecCoef(int n1, int n2, int n3, int ivar, CCTK_REAL *v, CCTK_REAL *cf)
{
  DECLARE_CCTK_PARAMETERS;
